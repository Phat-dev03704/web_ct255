"""
VRPTW Solver using Sweep Algorithm
Giải bài toán VRP với cửa sổ thời gian bằng Thuật toán quét

Thuật toán Heuristic dựa trên góc phương vị (polar angle)
"""
...
class SweepVRPTWSolver:
    """
    Class giải bài toán VRPTW bằng Sweep Algorithm
    """
    ...
    def __init__(self, dataset_path, vehicle_capacity=200, max_vehicles=25):
        ...
        # Tính góc phương vị cho mỗi khách hàng
        self.polar_angles = self._calculate_polar_angles()
    ...
    def _calculate_polar_angles(self):
        """
        Tính góc phương vị (polar angle) của mỗi khách hàng so với depot
        Góc được tính theo chiều kim đồng hồ từ trục dương x
        """
        print("Tính toán góc phương vị cho các khách hàng...")
        angles = {}
        ...
        for idx in range(len(self.customers)):
            customer_id = idx + 1
            customer_x = self.customers.loc[idx, 'XCOORD.']
            customer_y = self.customers.loc[idx, 'YCOORD.']
            
            # Tính góc sử dụng atan2 (trả về góc từ -π đến π)
            dx = customer_x - depot_x
            dy = customer_y - depot_y
            angle = math.atan2(dy, dx)
            
            # Chuyển sang độ (0 đến 360)
            angle_degrees = math.degrees(angle)
            if angle_degrees < 0:
                angle_degrees += 360
            
            angles[customer_id] = angle_degrees
        
        print(f"✓ Đã tính góc phương vị cho {len(angles)} khách hàng")
        return angles
    ...
    def _optimize_route_order(self, route):
        """
        Tối ưu thứ tự khách hàng trong route bằng nearest neighbor
        """
        if len(route) <= 1:
            return route
        ...
        if self._check_time_window_feasibility(optimized):
            return optimized
        else:
            return route  # Giữ nguyên nếu không thỏa mãn
    ...
    def solve(self, start_angle=0):
        """
        Giải bài toán bằng Sweep Algorithm
        
        Args:
            start_angle: Góc bắt đầu quét (độ, mặc định 0)
        """
        print(f"\n{'='*80}")
        print(f"XÂY DỰNG GIẢI PHÁP BẰNG SWEEP ALGORITHM")
        print(f"{'='*80}")
        ...
        print(f"Góc bắt đầu quét: {start_angle}°")
        print(f"{'='*80}\n")
        
        start_time = time.time()
        
        # Bước 1: Sắp xếp khách hàng theo góc phương vị
        print("Sắp xếp khách hàng theo góc phương vị (theo chiều kim đồng hồ)...")
        
        # Điều chỉnh góc dựa trên start_angle
        adjusted_angles = {}
        for customer_id, angle in self.polar_angles.items():
            adjusted = angle - start_angle
            if adjusted < 0:
                adjusted += 360
            adjusted_angles[customer_id] = adjusted
        
        # Sắp xếp theo góc tăng dần
        sorted_customers = sorted(adjusted_angles.keys(), key=lambda x: adjusted_angles[x])
        
        print(f"✓ Đã sắp xếp {len(sorted_customers)} khách hàng")
        print(f"  - Khách hàng đầu tiên: {sorted_customers[0]} (góc: {adjusted_angles[sorted_customers[0]]:.2f}°)")
        print(f"  - Khách hàng cuối cùng: {sorted_customers[-1]} (góc: {adjusted_angles[sorted_customers[-1]]:.2f}°)")
        
        # Bước 2: Tạo routes bằng cách quét theo góc
        print("\nTạo routes bằng cách quét theo góc...")
        routes = []
        current_route = []
        current_load = 0
        ...
        for customer_id in sorted_customers:
            ...
            # Kiểm tra nếu thêm khách hàng này có vượt capacity không
            if current_load + demand <= self.vehicle_capacity:
                # Thử thêm vào route hiện tại
                test_route = current_route + [customer_id]
                
                # Kiểm tra time window
                if self._check_time_window_feasibility(test_route):
                    current_route.append(customer_id)
                    current_load += demand
                else:
                    # Không thỏa mãn time window -> tạo route mới
                    if current_route:
                        routes.append(current_route)
                    current_route = [customer_id]
                    current_load = demand
            else:
                # Vượt capacity -> tạo route mới
                if current_route:
                    routes.append(current_route)
                current_route = [customer_id]
                current_load = demand
            ...
            if len(routes) >= self.max_vehicles:
                # Đã đạt max vehicles, gộp khách hàng còn lại vào route cuối
                if current_route and customer_id not in current_route:
                    current_route.append(customer_id)
        ...
        print(f"\n✓ Hoàn thành quét!")
        print(f"  - Số routes ban đầu: {len(routes)}")
        
        # Bước 3: Tối ưu thứ tự trong mỗi route
        print("\nTối ưu thứ tự khách hàng trong mỗi route...")
        optimized_routes = []
        for idx, route in enumerate(routes):
            optimized = self._optimize_route_order(route)
            optimized_routes.append(optimized)
            print(f"  Route {idx+1}: {len(route)} khách hàng")
        ...
        print(f"{'='*80}")
        print(f"Trạng thái: Feasible (Sweep Heuristic)")
    ...
    def visualize_solution_comprehensive(self, save=False):
        ...
        Bao gồm: Routes với góc quét, Thống kê xe, Phân bố demand
        """
        ...
        # ============= 1. VẼ CÁC TUYẾN ĐƯỜNG VỚI GÓC QUÉT (CHÍNH - LỚN) =============
        ...
        # Vẽ các đường tia góc quét (mờ)
        max_dist = max(self.distance_matrix[0])
        for angle in range(0, 360, 45):
            rad = math.radians(angle)
            end_x = depot_x + max_dist * math.cos(rad)
            end_y = depot_y + max_dist * math.sin(rad)
            ax1.plot([depot_x, end_x], [depot_y, end_y], 
        ...
        for idx, (route_id, info) in enumerate(self.solution.items()):
            ...
            # Label cho route với góc trung bình
            if len(info['route']) > 0:
                ...
                # Tính góc trung bình của route
                route_angles = [self.polar_angles[c] for c in info['route']]
                avg_angle = sum(route_angles) / len(route_angles)
        ...
        ax1.set_title('① Các tuyến đường (Sweep Algorithm - Quét theo góc)',
        ...
        fig.suptitle(
            f'KẾT QUẢ SWEEP ALGORITHM - {self.dataset_name}\n'
        ...
        if save:
            ...
            output_path = output_dir / f'{self.dataset_name}_sweep_solution.png'
    ...
    def save_solution(self, solve_time=None, status=None):
        ...
        output_dir.mkdir(parents=True, exist_ok=True)
        output_path = output_dir / f'{self.dataset_name}_sweep_solution.txt'
        ...
        with open(str(output_path), 'w', encoding='utf-8') as f:
            f.write("="*80 + "\n")
            f.write(f"BÁO CÁO KẾT QUẢ SWEEP ALGORITHM\n")
            ...
            f.write("="*80 + "\n")
            f.write(f"{'Xe':<8} {'Số KH':<10} {'Quãng đường':<15} {'Tải trọng':<15} {'% Capacity':<12} {'Góc TB':<10}\n")
            f.write("-"*80 + "\n")
            ...
            for route_id, info in self.solution.items():
                ...
                f.write(f"{route_id+1:<8} {info['num_customers']:<10} {info['distance']:<15.2f} "
                       f"{info['load']:<15} {capacity_pct:<12.1f} {avg_angle:<10.1f}\n")
            f.write("\n")
            ...
            for route_id, info in self.solution.items():
                ...
                f.write(f"\nChi tiết khách hàng:\n")
                f.write(f"  {'STT':<6} {'KH':<6} {'Góc':<10} {'Nhu cầu':<10} {'Ready Time':<12} {'Due Date':<12} {'Service Time':<12}\n")
                f.write(f"  {'-'*80}\n")
                
                for idx, customer_id in enumerate(info['route'], 1):
                    cust = self.customers.loc[customer_id-1]
                    angle = self.polar_angles[customer_id]
                    f.write(f"  {idx:<6} {customer_id:<6} {angle:<10.1f} {cust['DEMAND']:<10} "
...
def main():
    """Hàm chính để chạy Sweep solver"""
    ...
    print("="*80)
    print("SWEEP ALGORITHM FOR VRPTW")
    print("="*80)
    print("✓ Thuật toán Heuristic dựa trên góc phương vị")
    print("✓ Quét theo chiều kim đồng hồ từ depot")
    print("✓ Có thể giải toàn bộ dataset (100+ khách hàng)")
    print("✓ Rất nhanh và hiệu quả cho bài toán có cấu trúc cluster")
    print("="*80)
    ...
    # Giải bài toán (bắt đầu từ góc 0°)
