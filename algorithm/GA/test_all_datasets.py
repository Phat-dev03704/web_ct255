"""
Test Genetic Algorithm solver với tất cả datasets Solomon
...
from ga_vrptw_solver import GeneticAlgorithmVRPTWSolver


def test_all_datasets():
    """Test tất cả datasets với Genetic Algorithm"""
    
    print("="*80)
    print("TEST GENETIC ALGORITHM VỚI TẤT CẢ DATASETS SOLOMON")
    print("="*80)
    print("📊 Tổng số datasets: 56")
    print("🧬 Phương pháp: Genetic Algorithm (Evolution-based Metaheuristic)")
            ...
            try:
                ...
                if result and result['status']:
                    ...
                    results.append({
                        ...
                        'Generations': result['generations'],
                        'Improvements': result['improvements'],
                        'Final Fitness': f"{result.get('final_fitness', 0):.6f}",
                        'Status': 'Success'
                    })
    ...
    if success_results:
        ...
        for category in categories.keys():
            ...
            if len(cat_data) > 0:
                ...
                print(f"  Trung bình generations: {cat_data['Generations'].mean():.0f}")
    ...
    with open(report_path, 'w', encoding='utf-8') as f:
        f.write("="*80 + "\n")
        f.write("BÁO CÁO TEST GENETIC ALGORITHM - TẤT CẢ DATASETS\n")
        f.write("="*80 + "\n\n")
        ...
        f.write("  - Crossover: Order Crossover (OX) - lai ghép 2 parents\n")
        f.write("  - Mutation: Swap + Reverse (2-opt style)\n")
        f.write("  - Elitism: Giữ lại best individuals qua các thế hệ\n")
        f.write("  - Tự động điều chỉnh hyperparameters theo kích thước bài toán\n\n")
        
        f.write("Hyperparameters:\n")
        f.write("  - Population size: 50-150 (tùy kích thước)\n")
        f.write("  - Max generations: 100-200 (tùy kích thước)\n")
        f.write("  - Crossover rate: 0.8 (80% cặp lai ghép)\n")
        f.write("  - Mutation rate: 0.2 (20% cá thể đột biến)\n")
        f.write("  - Elite size: 5-15 (giữ lại best individuals)\n")
        f.write("  - Tournament size: 3 (selection pressure)\n\n")
        ...
        if success_results:
            ...
            for category in categories.keys():
                ...
                if len(cat_data) > 0:
                    ...
                    f.write(f"  Trung bình generations: {cat_data['Generations'].mean():.0f}\n\n")
