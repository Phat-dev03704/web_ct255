"""
Script để test MILP solver trên tất cả 56 datasets
...
# Import solver
from milp_vrptw_solver import MILPVRPTWSolver


def test_all_datasets():
    """Test MILP solver trên tất cả 56 datasets"""
    
    # Đường dẫn đến thư mục dataset - Sửa đường dẫn cho đúng
    # Từ code/algorithm/MILP/ lên code/ rồi vào dataset/
    current_dir = Path(__file__).parent  # algorithm/MILP/
    ...
    print("="*100)
    print("TEST MILP SOLVER TRÊN TẤT CẢ 56 DATASETS")
    print("="*100)
    print(f"Thư mục kết quả: {result_dir}")
    print(f"  - Images: {images_dir}")
    print(f"  - Text: {text_dir}")
    print("="*100)
    print(f"⚠️ LƯU Ý: MILP chỉ giải 25 khách hàng đầu tiên của mỗi dataset")
    print(f"   Thời gian giải: 1 phút/dataset")
    print(f"   Nếu không giải được, sẽ dùng Nearest Neighbor")
    print("="*100)
        ...
        for dataset_name in files:
            ...
            try:
                ...
                solver = MILPVRPTWSolver(
                    ...
                    max_vehicles=10,
                    max_customers=25  # Giới hạn 25 khách hàng
                )
    ...
    with open(str(report_path), 'w', encoding='utf-8') as f:
        f.write("="*100 + "\n")
        f.write("BÁO CÁO TEST MILP SOLVER TRÊN 56 DATASETS\n")
        f.write("="*100 + "\n\n")
