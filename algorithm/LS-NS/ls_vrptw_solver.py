"""
VRPTW Solver using Local Search / Neighborhood Search
Giải bài toán VRP với cửa sổ thời gian bằng Tìm kiếm cục bộ

Thuật toán sử dụng các phép biến đổi lân cận:
- 2-opt: Đảo ngược đoạn trong route
- Relocate: Di chuyển khách hàng giữa các routes
- Exchange: Hoán đổi khách hàng giữa các routes
- Cross: Hoán đổi chuỗi giữa hai routes
"""
...
class LocalSearchVRPTWSolver:
    """
    Class giải bài toán VRPTW bằng Local Search
    """
    ...
    def _two_opt_within_route(self, route):
        """
        Áp dụng 2-opt trong một route
        Đảo ngược đoạn [i+1, j] trong route
        """
        if len(route) < 4:
            return route
    ...
    def _relocate_move(self, routes):
        """
        Di chuyển một khách hàng từ route này sang route khác
        Trả về routes mới nếu cải thiện, None nếu không
        """
        best_routes = None
    ...
    def _exchange_move(self, routes):
        """
        Hoán đổi hai khách hàng giữa hai routes
        Trả về routes mới nếu cải thiện, None nếu không
        """
        best_routes = None
        best_distance = self._calculate_total_distance(routes)
    ...
    def _cross_move(self, routes):
        """
        Hoán đổi đoạn giữa hai routes (2-opt* between routes)
        Trả về routes mới nếu cải thiện, None nếu không
        """
        best_routes = None
        best_distance = self._calculate_total_distance(routes)
        ...
        for i, route_i in enumerate(routes):
            ...
            for j, route_j in enumerate(routes):
                if i >= j or len(route_j) < 2:
                    continue
                
                # Thử các điểm cắt
                for cut_i in range(1, len(route_i)):
                    for cut_j in range(1, len(route_j)):
                        # Tạo routes mới bằng cách hoán đổi đuôi
                        new_route_i = route_i[:cut_i] + route_j[cut_j:]
                        new_route_j = route_j[:cut_j] + route_i[cut_i:]
                        
                        # Kiểm tra capacity
                        load_i = self._calculate_route_load(new_route_i)
                        load_j = self._calculate_route_load(new_route_j)
                        
                        if load_i > self.vehicle_capacity or load_j > self.vehicle_capacity:
                            continue
    ...
    def solve(self, time_limit=60, max_iterations=1000):
        """
        Giải bài toán bằng Local Search
        ...
        """
        print(f"\n{'='*80}")
        print(f"XÂY DỰNG GIẢI PHÁP BẰNG LOCAL SEARCH")
        print(f"{'='*80}")
        ...
        while iterations < max_iterations:
            ...
            # Áp dụng 2-opt trong mỗi route
            new_routes = []
            for route in current_routes:
                if route:
                    optimized_route = self._two_opt_within_route(route)
                    new_routes.append(optimized_route)
                else:
            ...
            # Thử các phép biến đổi giữa routes
            moves = [
                ('Relocate', self._relocate_move),
                ('Exchange', self._exchange_move),
                ('Cross', self._cross_move)
            ]
            
            random.shuffle(moves)  # Ngẫu nhiên hóa thứ tự thử
            
            for move_name, move_func in moves:
                new_routes = move_func(current_routes)
            ...
            if current_distance < best_distance:
                ...
                print(f"  Vòng {iterations}: Cải thiện! Khoảng cách = {best_distance:.2f}")
                no_improvement_count = 0
            else:
                no_improvement_count += 1
            
            # Dừng sớm nếu không cải thiện trong 100 vòng
            if no_improvement_count >= 100:
                print(f"\n🛑 Dừng sớm: Không cải thiện trong 100 vòng liên tiếp")
                break
            
            # In tiến độ mỗi 100 vòng
            if iterations % 100 == 0:
                print(f"  Vòng {iterations}: Distance = {current_distance:.2f}, Best = {best_distance:.2f}")
        ...
        print(f"{'='*80}")
        print(f"Trạng thái: Feasible (Local Search)")
        ...
        print(f"Cải thiện: {((current_distance - best_distance) / current_distance * 100):.2f}%")
    ...
    def visualize_solution_comprehensive(self, save=False):
        ...
        Bao gồm: Routes, Thống kê xe, Convergence curve
        """
        ...
        ax1.set_title('① Các tuyến đường (Local Search - Tìm kiếm cục bộ)',
        ...
        fig.suptitle(
            f'KẾT QUẢ LOCAL SEARCH ALGORITHM - {self.dataset_name}\n'
        ...
        if save:
            ...
            output_path = output_dir / f'{self.dataset_name}_ls_solution.png'
    ...
    def save_solution(self, solve_time=None, status=None, iterations=None, improvements=None):
        ...
        output_dir.mkdir(parents=True, exist_ok=True)
        output_path = output_dir / f'{self.dataset_name}_ls_solution.txt'
        ...
        with open(str(output_path), 'w', encoding='utf-8') as f:
            f.write("="*80 + "\n")
            f.write(f"BÁO CÁO KẾT QUẢ LOCAL SEARCH ALGORITHM\n")
            f.write(f"Neighborhood Search for VRPTW\n")
            f.write("="*80 + "\n\n")
            ...
            # PHƯƠNG PHÁP LOCAL SEARCH
            f.write("="*80 + "\n")
            f.write("PHƯƠNG PHÁP LOCAL SEARCH\n")
            f.write("="*80 + "\n")
            f.write("Các phép biến đổi lân cận:\n")
            f.write("  1. 2-opt: Đảo ngược đoạn trong route\n")
            f.write("  2. Relocate: Di chuyển khách hàng giữa routes\n")
            f.write("  3. Exchange: Hoán đổi khách hàng giữa routes\n")
            f.write("  4. Cross: Hoán đổi chuỗi giữa hai routes\n")
            f.write("\n")
...
def main():
    """Hàm chính để chạy Local Search solver"""
    ...
    print("="*80)
    print("LOCAL SEARCH / NEIGHBORHOOD SEARCH FOR VRPTW")
    print("="*80)
    print("✓ Thuật toán cải tiến solution thông qua các phép biến đổi lân cận")
    print("✓ Sử dụng: 2-opt, Relocate, Exchange, Cross")
    print("✓ Có thể giải toàn bộ dataset (100+ khách hàng)")
    print("✓ Chất lượng solution tốt, thời gian hợp lý")
    print("="*80)
