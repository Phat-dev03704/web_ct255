"""
Test Simulated Annealing solver với tất cả datasets Solomon
...
from sa_vrptw_solver import SimulatedAnnealingVRPTWSolver


def test_all_datasets():
    """Test tất cả datasets với Simulated Annealing"""
    
    print("="*80)
    print("TEST SIMULATED ANNEALING VỚI TẤT CẢ DATASETS SOLOMON")
    print("="*80)
    print("📊 Tổng số datasets: 56")
    print("🌡️ Phương pháp: Simulated Annealing (Temperature-based Metaheuristic)")
    ...
    for category, files in categories.items():
        ...
        for file_name in files:
            ...
            try:
                ...
                print(f"  - Initial temperature: {hyperparams['initial_temperature']:.2f}")
                print(f"  - Final temperature: {hyperparams['final_temperature']:.4f}")
                print(f"  - Cooling rate: {hyperparams['cooling_rate']}")
                print(f"  - Iterations/temp: {hyperparams['max_iterations_per_temp']}")
                ...
                if result and result['status']:
                    ...
                    results.append({
                        ...
                        'Iterations': result['iterations'],
                        'Improvements': result['improvements'],
                        'Acceptance Rate': f"{result.get('acceptance_rate', 0):.2%}",
                        'Status': 'Success'
                    })
    ...
    with open(report_path, 'w', encoding='utf-8') as f:
        f.write("="*80 + "\n")
        f.write("BÁO CÁO TEST SIMULATED ANNEALING - TẤT CẢ DATASETS\n")
        f.write("="*80 + "\n\n")
        ...
        f.write("  - Công thức acceptance: P = exp(-(new_cost - old_cost) / T)\n")
        f.write("  - Tự động điều chỉnh hyperparameters theo kích thước bài toán\n\n")
        
        f.write("Hyperparameters:\n")
        f.write("  - Initial temperature: Tự động (10x average distance)\n")
        f.write("  - Final temperature: 0.1% của initial\n")
        f.write("  - Cooling rate: 0.97-0.99 (tùy kích thước)\n")
        f.write("  - Iterations/temp: 100-200 (tùy kích thước)\n")
        f.write("  - Neighborhood: Relocate, Exchange, 2-opt\n\n")
